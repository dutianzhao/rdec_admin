...
public class SysInfo
{
	...
	public SystemCom systemComput()
	{
		SystemCom systemCom = new SystemCom();
		
		systemCom.setJdkVersion(System.getProperty("java.version"));
		systemCom.setSysName(System.getProperty("os.name"));
		systemCom.setOsArch(System.getProperty("os.arch"));
		
		return systemCom;
	}

	// c)CPU的用户使用量、系统使用剩余量、总的剩余量、总的使用占用量等（单位：100%）
	public List<SysCpu> sysCpuPerc()
	{
		List<SysCpu> listSysCpu = new ArrayList<SysCpu>();
		...
		for (int i = 0; i < cpuList.length; i++)
		{
			...
			sysCpu.setId(i + 1);
			sysCpu.setIdleCpu(CpuPerc.format(cpu.getIdle()));
			sysCpu.setSysCpu(CpuPerc.format(cpu.getSys()));
			sysCpu.setTotalUsr(CpuPerc.format(cpu.getCombined()));
			sysCpu.setType(infos[i].getModel());
			sysCpu.setUserCpu(CpuPerc.format(cpu.getUser()));
			listSysCpu.add(sysCpu);
		}
		return listSysCpu;
	}
	...
	public SysMemory getPhysicalMemory()
	{
		...
		try
		{
			...
			sysMemory.setId(1);
			sysMemory.setMemoryUsr(df.format((float) mem.getUsed() / 1024 / 1024 / 1024) + "G");
			sysMemory.setSurplus(df.format((float) mem.getFree() / 1024 / 1024 / 1024) + "G");
			sysMemory.setMemoryNum(df.format((float) mem.getTotal() / 1024 / 1024 / 1024) + "G");
	...
	public List<SysDisk> systemDisk() throws Exception
	{
		List<SysDisk> listDisk = new ArrayList<SysDisk>();
		...
		for (int i = 0; i < fslist.length; i++)
		{
			...
			sysDisk.setNameDisk(fs.getDirName());
			sysDisk.setTypeDisk(fs.getSysTypeName());
			...
			case 2: // TYPE_LOCAL_DISK : 本地硬盘
				...
				sysDisk.setTotalDisk(df.format((float) usage.getTotal() / 1024 / 1024) + "G");
				sysDisk.setFreeDisk(df.format((float) usage.getFree() / 1024 / 1024) + "G");
				sysDisk.setUseDisk(df.format((float) usage.getUsed() / 1024 / 1024) + "G");
				double usePercent = usage.getUsePercent() * 100D;
				sysDisk.setUsageDisk(df.format(usePercent) + "%");
				break;
